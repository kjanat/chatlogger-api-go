...
func (b *UserBuilder) WithTimestamps(createdAt, updatedAt time.Time, lastLoginAt *time.Time) *UserBuilder {
...
func (f *TestDataFactory) CreateOrganizationWithUsers(orgName string, userCount int) (*domain.Organization, []*domain.User) {
...
func (f *TestDataFactory) CreateChatWithMessages(user *domain.User, title string, messageCount int) (*domain.Chat, []*domain.Message) {
	chat := NewChatBuilder().
		WithUser(user).
		WithTitle(title).
		Build()
...
func (f *TestDataFactory) CreateConversation(orgName string) (*domain.Organization, *domain.User, *domain.Chat, []*domain.Message) {
	org := NewOrganizationBuilder().
		WithName(orgName).
		WithSlug(fmt.Sprintf("%s-slug", orgName)).
		Build()
